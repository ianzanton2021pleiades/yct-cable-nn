"""
[V2.3]DG_dataset_max2.5km.py - DirtyGenerator for <=2.5 km cable S11 datasets.
...
def defect_type_policy(profile: str, total_length: float) -> dict:
    ...
    return {
        ...
        # The first four entries reproduce the original V2.3 families.  The
...
def dirty_params_for_profile(profile: str, rng: np.random.RandomState) -> DirtyParams:
    ...
    return DirtyParams(
        ...
        field_step_wander_scale=float(rng.uniform(0.78, 1.22)),
...
def apply_field_baseline_texture_s11(
    ...
    """Add continuous weak field-cable IFFT texture over the whole cable.

    This is not the slow step wander and not a set of isolated reflectors.  It is
    a correlated zero-mean micro-mismatch field with a smooth distance envelope,
    a non-zero remote floor, and additional attenuation after wet regions.
    """
    if params.profile != "field":
        return out
    result = np.asarray(out, dtype=np.complex128).copy()
    ...
    terminal = effective_terminal_phase_length_m(cable)
    terminal_peak = _response_peak(
        distance, impulse, terminal - max(18.0, 0.012 * length), terminal + max(25.0, 0.018 * length)
    )
    front_peak = _response_peak(distance, impulse, 0.0, min(24.0, 0.08 * length + 8.0))
    reference_peak = max(terminal_peak, 0.18 * front_peak, 2.0e-4)
    band_scale = float(np.clip(float(freq_hz[-1]) / 1.0e9, 0.40, 1.0))
    target_early_rms = float(np.clip(
        reference_peak * rng.uniform(0.0065, 0.0175) * band_scale,
        7.0e-6 * band_scale,
        2.8e-4 * band_scale,
    ))
    ...
    n = len(distance)
    dx = max(float(np.median(np.diff(distance[: min(n, 4096)]))), 1e-5)
    sigma_fast = float(np.clip(rng.uniform(0.35, 0.95) / dx, 1.0, 24.0))
    radius_fast = max(4, int(math.ceil(3.5 * sigma_fast)))
    xf = np.arange(-radius_fast, radius_fast + 1, dtype=np.float64)
    k_fast = np.exp(-0.5 * np.square(xf / sigma_fast))
    k_fast /= max(float(np.sqrt(np.sum(k_fast * k_fast))), 1e-12)
    fast = np.convolve(rng.normal(size=n), k_fast, mode="same")

    sigma_slow = float(np.clip(rng.uniform(1.4, 4.8) / dx, 4.0, 120.0))
    radius_slow = max(8, int(math.ceil(3.0 * sigma_slow)))
    xs = np.arange(-radius_slow, radius_slow + 1, dtype=np.float64)
    k_slow = np.exp(-0.5 * np.square(xs / sigma_slow))
    k_slow /= max(float(np.sqrt(np.sum(k_slow * k_slow))), 1e-12)
    slow = np.convolve(rng.normal(size=n), k_slow, mode="same")
    texture = 0.86 * fast + 0.14 * slow

    start_m = float(np.clip(14.0 + 0.006 * length, 16.0, 34.0))
    reference = (distance >= start_m + 8.0) & (distance <= min(0.40 * length + 40.0, length - 25.0))
    if int(reference.sum()) < 30:
        reference = (distance >= start_m) & (distance <= max(start_m + 20.0, 0.70 * length))
    texture -= float(np.nanmedian(texture[reference]))
    std = float(np.nanstd(texture[reference]))
    if not np.isfinite(std) or std <= 1e-12:
        return result
    texture /= std

    decay_m = rng.uniform(0.34, 0.58) * length + rng.uniform(55.0, 120.0)
    remote_floor = rng.uniform(0.09, 0.22)
    envelope = remote_floor + (1.0 - remote_floor) * np.exp(-distance / max(decay_m, 1.0))
    entrance = _smoothstep01((distance - start_m) / max(rng.uniform(7.0, 16.0), 1e-6))
    envelope *= entrance

    # Wet regions attenuate all later micro-reflections smoothly instead of
    # producing an abrupt texture discontinuity.
    wet_factor = np.ones_like(distance, dtype=np.float64)
    for region in _moisture_regions(cable):
        start = float(region["start"])
        end = float(region["end"])
        severity = float(np.clip(region.get("severity", 0.62), 0.30, 0.90))
        width = max(end - start, 1.0)
        through = _smoothstep01((distance - start) / width)
        retained = math.exp(-severity * rng.uniform(0.26, 0.52))
        wet_factor *= 1.0 - (1.0 - retained) * through
    envelope *= wet_factor

    target = texture * envelope
    event_positions = [float(p) for p in getattr(cable, "joint_positions", [])]
    event_positions += [float(d["position"]) for d in cable.defect_info]
    for event in event_positions:
        notch_width = float(np.clip(0.0035 * length, 2.0, 10.0))
        target *= 1.0 - 0.62 * np.exp(-0.5 * np.square((distance - event) / notch_width))

    current = _rms(target[reference])
    if current <= 1e-12:
        return result
    target *= target_early_rms / current
    after = distance > length
    target[after] *= np.exp(-(distance[after] - length) / max(18.0, 0.04 * length))
    active = distance <= length + max(25.0, 0.06 * length)
    target[~active] = 0.0
...
def build_field_near_end_texture_s11(
    ...
    """Create a measured-like field clamp spectrum and dense port burst.

    V2.3 originally concentrated too much energy in one 6-15 m mode and made
    the genuine high-frequency 0-20 m cluster disappear.  This revision keeps
    the smooth coherent carrier that fixed Re/Im S11, but redistributes part of
    its energy into many compact, high-bandwidth clamp echoes.  The resulting
    spectrum remains smooth at the sweep-bin scale while its IFFT again has the
    dense alternating front-end peaks seen in field measurements.
    """
    del window
    freq_hz = np.asarray(freq_hz, dtype=np.float64)
    omega = 2.0 * math.pi * freq_hz
    ...
    # Smooth low-frequency carrier / clamp mode.  It is no longer allowed to
    # dominate the entire IFFT by itself.
    main_delay = float(np.clip(rng.uniform(5.8, 11.8) + 0.0008 * min(length, 1800.0), 5.2, 13.6))
    main_corner = rng.uniform(115e6, 280e6)
    main_amp = rng.uniform(0.12, 0.25) * factor * family_scale
    correction += (
        main_amp
        * np.exp(1j * rng.uniform(-math.pi, math.pi))
        * causal_lowpass(freq_hz, main_corner, rng.uniform(0.82, 1.28))
        * np.exp(-2j * omega * main_delay / velocity)
    )

    # Broad almost-direct path gives the 0 m complex offset and a visible first
    # edge without replacing the dense ringing train.
    correction += (
        rng.uniform(0.075, 0.16)
        * factor
        * family_scale
        * np.exp(1j * rng.uniform(-math.pi, math.pi))
        * causal_lowpass(freq_hz, rng.uniform(420e6, 1.05e9), rng.uniform(0.48, 0.92))
        * np.exp(-2j * omega * rng.uniform(0.03, 0.48) / velocity)
    )

    # A few coherent neighbours retain the accepted V2.3 envelope undulation.
    for _ in range(int(rng.randint(2, 4))):
        delay = float(np.clip(main_delay + rng.uniform(-3.2, 4.4), 1.0, 17.5))
        correction += (
            main_amp
            * rng.uniform(0.055, 0.15)
            * np.exp(1j * rng.uniform(-math.pi, math.pi))
            * causal_lowpass(freq_hz, main_corner * rng.uniform(0.72, 1.60), rng.uniform(0.52, 1.12))
            * np.exp(-2j * omega * delay / velocity)
        )

    # Dense, irregular high-frequency clamp echoes.  Alternation plus mild
    # jitter avoids isolated one-sided peaks and a synthetic periodic comb.
    knee_m = float(np.clip(10.0 + 0.0050 * length, 11.0, 18.0))
    decay_m = rng.uniform(4.8, 8.2)
    position = rng.uniform(0.16, 0.38)
    echo_index = 0
    while position < knee_m:
        amplitude = rng.uniform(0.050, 0.18) * math.exp(-position / decay_m)
        sign = -1.0 if echo_index % 2 else 1.0
        sign *= rng.choice([0.82, 1.0, 1.16])
        correction += (
            sign
            * amplitude
            * factor
            * family_scale
            * causal_lowpass(freq_hz, rng.uniform(330e6, 1.25e9), rng.uniform(0.30, 0.82))
            * np.exp(-2j * omega * position / velocity)
        )
        position += rng.uniform(0.16, 0.42) * (1.0 + 0.010 * position)
        echo_index += 1

    # Compact transition only; do not reintroduce the old sparse 20-80 m tail.
    transition_end = knee_m + rng.uniform(3.0, 6.0)
    position = knee_m + rng.uniform(0.12, 0.30)
    tail_index = 0
    while position < transition_end:
        progress = (position - knee_m) / max(transition_end - knee_m, 1e-6)
        taper = (1.0 - _smoothstep01(progress)) * math.exp(-1.8 * progress)
        correction += (
            (-1.0 if tail_index % 2 else 1.0)
            * rng.uniform(0.012, 0.045)
            * taper
            * factor
            * family_scale
            * causal_lowpass(freq_hz, rng.uniform(220e6, 760e6), rng.uniform(0.45, 1.0))
            * np.exp(-2j * omega * position / velocity)
        )
        position += rng.uniform(0.20, 0.50)
        tail_index += 1

    # Broad contact resonances enrich the smooth S11 trajectory.
    for _ in range(int(rng.randint(2, 5))):
        correction += (
            ...
            * family_scale
            * np.exp(1j * rng.uniform(-math.pi, math.pi))
            * causal_resonant_mode(freq_hz, rng.uniform(45e6, 430e6), rng.uniform(0.8, 3.8))
            * np.exp(-2j * omega * rng.uniform(0.15, 3.8) / velocity)
        )

    low_mask = freq_hz <= min(80e6, float(freq_hz[-1]))
    current_low = float(np.nanmedian(np.abs(correction[low_mask]))) if int(low_mask.sum()) else 0.0
    target_low = float(np.clip(rng.uniform(0.32, 0.50) * factor * family_scale, 0.27, 0.58))
    if np.isfinite(current_low) and current_low > 1e-12:
        correction *= float(np.clip(target_low / current_low, 0.45, 1.65))
    # Preserve the measured low-frequency-dominant S11 envelope.  A non-zero
    # high-frequency floor keeps the dense port cluster sharp without letting
    # the 600-1000 MHz band carry the same energy as 20-100 MHz.
    rolloff = 0.26 + 0.74 / np.sqrt(1.0 + np.power(freq_hz / 185e6, 1.55))
    correction *= rolloff
    return correction
...
def _field_cable_visibility_transfer(
    ...
    """Causal transfer that makes the long cable a small ripple on fixture S11."""
    length = max(float(cable.total_length), 30.0)
    corner = float(np.clip(
        rng.uniform(0.95e6, 2.2e6) * (600.0 / length) ** 0.72,
        0.75e6,
        11.0e6,
    ))
    # V2.3 attenuated the physical cable to 0.1-8%, which erased terminal and
    # defect returns.  Retain a modest coherent backbone while the clamp still
    # dominates the measured S11 envelope.
    low_gain = rng.uniform(0.10, 0.22)
    floor = rng.uniform(0.012, 0.045)
    order = rng.uniform(0.72, 1.05)
...
def apply_field_background_clutter_s11(
    ...
    """Add low-frequency field-cable wander without copying template events.

    Measured field curves are not straight ideal baselines: after the strong
    clamp burst they exhibit a slowly varying drift over tens to hundreds of
    metres.  The correction below is a newly drawn causal step trajectory.  Its
    derivative becomes a broad, low-amplitude impulse background, while all
    labelled reflection positions remain controlled by the cable model.
    """
    if params.profile != "field" or params.field_step_wander_scale <= 0.0:
        return out
    try:
        distance, _, step, _ = s11_to_responses(
            freq_hz, out, epsr=cable.epsr, window=window
        )
    except Exception:
        return out
    ...
    length = float(cable.total_length)
    start_m = float(np.clip(20.0 + 0.010 * length, 24.0, 52.0))
    guard_m = float(np.clip(58.0 + 0.030 * length, 70.0, 145.0))
    end_m = min(terminal_m - guard_m, float(distance[-1]) * 0.92)
    if end_m <= start_m + 80.0:
        return out

    baseline_mask = (distance >= max(start_m, 0.05 * length)) & (
        distance <= min(end_m, 0.90 * length)
    )
    if int(baseline_mask.sum()) < 40:
        return out
    current_range = float(
        np.nanpercentile(step[baseline_mask], 95)
        - np.nanpercentile(step[baseline_mask], 5)
    )
    length_factor = float(np.clip((length / 1000.0) ** 0.18, 0.82, 1.18))
    target_range = (
        rng.uniform(4.0e-11, 8.5e-11)
        * length_factor
        * float(params.field_step_wander_scale)
    )
    # Strong physical defect morphology already provides sufficient
    # non-ideality; do not add another large independent trajectory.
    if current_range >= 0.92 * target_range:
        return out
    needed = float(np.sqrt(max(target_range * target_range - current_range * current_range, 0.0)))
    if needed < 8e-12:
        return out

    active = (distance >= start_m) & (distance <= end_m)
    x_active = distance[active]
    span = end_m - start_m
    corr_length = float(np.clip(42.0 + 0.045 * length, 48.0, 145.0))
    n_knots = int(np.clip(round(span / corr_length) + 2, 7, 28))
    xp = np.linspace(start_m, end_m, n_knots)
    yp = rng.normal(0.0, 1.0, size=n_knots)
    yp[0] = 0.0
    smooth_random = np.interp(x_active, xp, yp)
    smooth_window = int(np.clip(round(corr_length / max(np.median(np.diff(distance)), 1e-6)), 21, 801))
    if smooth_window % 2 == 0:
        smooth_window += 1
    smooth_random = smooth_array(smooth_random, smooth_window)
    smooth_random -= smooth_random[0]
    random_span = float(np.nanpercentile(smooth_random, 95) - np.nanpercentile(smooth_random, 5))
    if random_span > 1e-12:
        smooth_random *= (needed * rng.uniform(0.32, 0.58)) / random_span

    # Most real field traces have a gentle negative baseline drift.  The
    # stochastic component prevents this from becoming an artificial straight
    # line.
    total_drift = -needed * rng.uniform(0.58, 1.05)
    trend = total_drift * (x_active - start_m) / span
    local_step = trend + smooth_random
    enter = _smoothstep01((x_active - start_m) / max(18.0, 0.08 * span))
    local_step *= enter

    correction_step = np.zeros_like(distance, dtype=np.float64)
    correction_step[active] = local_step
    correction_step[distance > end_m] = float(local_step[-1])
    try:
...
def apply_moisture_response_s11(
    ...
    """Encode measured moisture morphology as a causal additive S11 kernel.

    The RLGC model supplies slower propagation, dielectric loss, and boundary
    reflections.  This calibrated kernel supplies the experimentally observed
    macroscopic signature that a finite wet section otherwise underestimates:
    a monotonic steeper decline through the wet region, a persistent lower
    post-wet level, and a post-wet slope close to (slightly flatter than) the
    healthy pre-wet slope.  No distance-domain output is edited.
    """
    ...
    for region in regions:
        ...
        if start >= terminal_m - 60.0:
            continue
        defect_type = str(region.get("type", "moisture_distributed"))
        severity = float(np.clip(region.get("severity", 0.64), 0.35, 0.85))

        dd = float(np.median(np.diff(distance[: min(len(distance), 2048)])))
        smooth_n = int(np.clip(round(24.0 / max(dd, 1e-6)), 31, 501))
        if smooth_n % 2 == 0:
            smooth_n += 1
        smooth_step = smooth_array(step, smooth_n)

        pre_margin = float(np.clip(0.10 * length, 25.0, 120.0))
        pre_width = float(np.clip(0.22 * length, 70.0, 260.0))
        pre_mask = (distance >= max(20.0, start - pre_width)) & (
            distance <= start - pre_margin
        )
        wet_margin = float(np.clip(0.12 * length, 15.0, 110.0))
        wet_mask = (distance >= start + wet_margin) & (
            distance <= end - wet_margin
        )
        post_start = end + float(np.clip(0.08 * length, 25.0, 100.0))
        post_end = min(
            terminal_m - float(np.clip(85.0 + 0.025 * cable.total_length, 95.0, 165.0)),
            end + float(np.clip(0.75 * length, 180.0, 620.0)),
        )
        post_mask = (distance >= post_start) & (distance <= post_end)

        pre_slope = _robust_slope(distance, smooth_step, pre_mask)
        post_slope = _robust_slope(distance, smooth_step, post_mask)
        if pre_slope is None:
            pre_slope = -2.5e-14
        if post_slope is None:
            post_slope = pre_slope

        line_mask = (distance >= max(20.0, 0.03 * cable.total_length)) & (
            distance <= min(terminal_m - 80.0, 0.92 * cable.total_length)
        )
        response_scale = max(
            float(np.nanpercentile(np.abs(smooth_step[line_mask]), 85))
            if int(line_mask.sum()) else 0.0,
            8.0e-11,
        )
        shape_scale = float(params.moisture_shape_scale)
        if defect_type == "moisture_local":
            drop = float(np.clip(
                response_scale * (0.38 + 0.52 * severity) * shape_scale,
                5.0e-11,
                1.75e-10,
            ))
            attack_m = float(np.clip(0.12 * length, 6.0, 32.0))
            release_m = float(np.clip(0.22 * length, 10.0, 55.0))
        else:
            length_gain = float(np.clip(np.sqrt(length / max(0.30 * cable.total_length, 120.0)), 0.82, 1.38))
            drop = float(np.clip(
                response_scale * (0.78 + 0.82 * severity) * length_gain * shape_scale,
                1.15e-10,
                3.80e-10,
            ))
            attack_m = float(np.clip(0.10 * length, 18.0, 85.0))
            release_m = float(np.clip(0.18 * length, 35.0, 135.0))

        # A normalized negative slope pulse integrates to a permanent drop.
        enter = _smoothstep01((distance - start) / max(attack_m, 1.0))
        leave = 1.0 - _smoothstep01(
            (distance - (end - release_m)) / max(release_m, 1.0)
        )
        slope_weight = enter * leave
        slope_weight[(distance < start) | (distance > end)] = 0.0
        integral = float(np.sum(slope_weight) * dd)
        if integral <= 1e-12:
            continue
        correction_step = -drop * np.cumsum(slope_weight) * dd / integral
        plateau = float(correction_step[np.searchsorted(distance, end, side="right") - 1])
        correction_step[distance > end] = plateau
        correction_step[distance < start] = 0.0

        # Cancel the physical model's frequent positive recovery and set the
        # downstream slope to about 65-85% of the healthy slope magnitude.
        target_post_slope = 0.72 * pre_slope
        if pre_slope > 0.0:
            target_post_slope = min(0.0, 0.35 * pre_slope)
        slope_delta = float(np.clip(target_post_slope - post_slope, -1.20e-12, 1.20e-12))
        anchor = end + max(18.0, 0.06 * length)
        safe_end = min(terminal_m - 95.0, end + max(210.0, min(720.0, 1.15 * length)))
        if safe_end > anchor + 60.0 and abs(slope_delta) > 2e-15:
            ramp = _smoothstep01((distance - anchor) / max(35.0, 0.12 * (safe_end - anchor)))
            trend = slope_delta * np.clip(distance - anchor, 0.0, safe_end - anchor) * ramp
            trend = np.clip(trend, -1.55e-10, 1.55e-10)
            trend[distance > safe_end] = float(trend[np.searchsorted(distance, safe_end, side="right") - 1])
            correction_step += trend
        ...
        # Finite wet sections naturally create a positive reflection at the
        # wet-to-healthy boundary.  In real water ingress that boundary is
        # diffuse and lossy, so the step normally stays on the lower level
        # instead of rebounding.  Measure any residual recovery after the first
        # correction and cancel only its positive component with a smooth,
        # persistent S11-domain offset.
        try:
            d_after, _, step_after, _ = s11_to_responses(
                freq_hz, result, epsr=cable.epsr, window=window
            )
            smooth_after = smooth_array(step_after, smooth_n)
            before_mask = (
                d_after >= max(start + 0.58 * length, end - max(150.0, 0.24 * length))
            ) & (d_after <= end - max(12.0, 0.025 * length))
            near_post_mask = (
                d_after >= end + max(18.0, 0.035 * length)
            ) & (
                d_after <= min(
                    terminal_m - 100.0,
                    end + max(120.0, 0.30 * length),
                )
            )
            if int(before_mask.sum()) >= 20 and int(near_post_mask.sum()) >= 20:
                x_before = d_after[before_mask]
                y_before = smooth_after[before_mask]
                _wet_tail_slope, wet_tail_intercept = np.polyfit(
                    x_before - end, y_before, 1
                )
                level_at_end = float(wet_tail_intercept)
                desired_post_slope = 0.72 * pre_slope
                if desired_post_slope > 0.0:
                    desired_post_slope = -min(1.5e-14, 0.15 * desired_post_slope)
                desired_post = level_at_end + desired_post_slope * (
                    d_after[near_post_mask] - end
                )
                recovery = float(np.nanmedian(
                    smooth_after[near_post_mask] - desired_post
                ))
                recovery = float(np.clip(recovery, 0.0, 1.20e-10))
                if recovery > 6.0e-12:
                    cancel_width = float(np.clip(0.16 * length, 28.0, 115.0))
                    cancel_start = end - 0.12 * cancel_width
                    cancel_step = -recovery * _smoothstep01(
                        (d_after - cancel_start) / cancel_width
                    )
                    cancel_step[d_after < cancel_start] = 0.0
                    cancel_kernel = step_shape_to_s11_kernel(
                        freq_hz, cable.epsr, cancel_step, window=window
                    )
                    result += cancel_kernel
        except Exception:
            pass
    return result
...
def ensure_terminal_visibility_s11(
    ...
    """Guarantee a visible, correctly signed terminal in the stored S11."""
    if len(freq_hz) < 128:
        return out
    result = np.asarray(out, dtype=np.complex128).copy()
    ...
    terminal_m = effective_terminal_phase_length_m(cable)
    length = float(cable.total_length)
    band_scale = float(np.clip(float(freq_hz[-1]) / 1.0e9, 0.38, 1.0))

    if is_rg58_profile(profile):
        corner = rng.uniform(260e6, 720e6)
        order = rng.uniform(1.20, 2.05)
        peak_ratio = rng.uniform(0.48, 0.78)
        width = 12.0
        minimum, maximum = 1.2e-4 * band_scale, 0.030 * band_scale
    else:
        if length < 500.0:
            corner = rng.uniform(65e6, 180e6)
            peak_ratio = rng.uniform(0.20, 0.36)
            width = 34.0
        elif length < 1500.0:
            corner = rng.uniform(28e6, 88e6)
            peak_ratio = rng.uniform(0.14, 0.26)
            width = 70.0
        else:
            corner = rng.uniform(15e6, 55e6)
            peak_ratio = rng.uniform(0.09, 0.19)
            width = 100.0
        order = rng.uniform(1.15, 2.15)
        if termination == "weak_open":
            peak_ratio *= 0.58
        elif termination == "short":
            peak_ratio *= 0.94
        minimum, maximum = 0.0040 * band_scale, 0.055 * band_scale

    omega = 2.0 * math.pi * freq_hz
    velocity = 299_792_458.0 / math.sqrt(max(float(cable.epsr), 1.1))
    unit = sign * causal_lowpass(freq_hz, corner, order) * np.exp(-2j * omega * terminal_m / velocity)
    try:
        distance, impulse, _, _ = s11_to_responses(freq_hz, result, epsr=cable.epsr, window=window)
        _, unit_impulse, _, _ = s11_to_responses(freq_hz, unit, epsr=cable.epsr, window=window)
    except Exception:
        return result
    front_mask = (distance >= 0.0) & (distance <= min(24.0, 0.08 * length + 8.0))
    front_values = np.abs(np.real(impulse[front_mask]))
    if int(front_mask.sum()):
        front_peak = max(
            float(np.nanpercentile(front_values, 99.5)),
            0.72 * float(np.nanmax(front_values)),
        )
    else:
        front_peak = 0.0
    body_mask = (distance >= max(30.0, 0.04 * length)) & (distance <= min(0.86 * length, terminal_m - 70.0))
    body_rms = _rms(np.real(impulse[body_mask])) if int(body_mask.sum()) else 0.0
    wet_path = max(_event_path_transmission(cable, terminal_m), 0.48 if profile == "field" else 0.34)
    desired = max(front_peak * peak_ratio * wet_path, body_rms * 22.0, minimum * wet_path)
    desired = float(np.clip(desired, minimum * wet_path, maximum))
    mask = (distance >= terminal_m - width) & (distance <= terminal_m + 1.7 * width)
    current = float(np.nanmax(sign * np.real(impulse[mask]))) if int(mask.sum()) else 0.0
    unit_peak = float(np.nanmax(sign * np.real(unit_impulse[mask]))) if int(mask.sum()) else 0.0
    if unit_peak <= 1e-14 or current >= 0.98 * desired:
        return result
    coefficient = float(np.clip((desired - current) / unit_peak, 0.0, 0.95))
    # Terminal visibility is a structural event, not nuisance clutter.  Add the
    # calibrated causal return directly and let the final |S11| safety limiter
    # handle the very small set of constructive-frequency bins.
    return result + coefficient * unit
...
def apply_dirty_model(
    ...
    # Establish the terminal reference first, then scale connector morphology
    # against the actual sample.  All additions remain in S11 and are therefore
    # exactly reproduced by the exported Client IFFT.
    ...
    # Joint/body additions can alter the terminal window by a few percent.
    out = ensure_terminal_visibility_s11(out, freq_hz, cable, params.profile, rng, window=window)
    ...
    if params.profile == "field":
        ...
        n = len(freq_hz)
        sigma_bins = float(rng.uniform(5.0, 16.0))
        ...
        corr_level = (
            ...
            + rng.uniform(2.5e-4, 9.0e-4)
            * params.multiplicative_scale
            * np.abs(out)
        )
        white_level = 2.5e-5 * params.additive_scale + 6.0e-5 * np.abs(out)
        out += corr * corr_level
...
def generate_band(
    ...
    # V2.3 invariant: these are the only response calculations.  No distance-
