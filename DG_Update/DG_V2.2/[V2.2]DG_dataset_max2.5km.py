"""
[V2.2]DG_dataset_max2.5km.py - DirtyGenerator for <=2.5 km cable S11 datasets.
...
def apply_bipolar_joint_model_s11(
    ...
    """Add finite-width positive/negative connector signatures.

    A physical joint is a short impedance excursion with an entry and a return
    boundary, not a delayed one-sided delta.  The two area-balanced lobes below
    reproduce the measured positive-then-negative impulse while distance-
    dependent broadening and attenuation keep later joints weaker and wider.
    """
    ...
    front_peak = _response_peak(distance, impulse, 0.0, min(8.0, 0.12 * length + 2.0))
    terminal = effective_terminal_phase_length_m(cable)
    terminal_half_width = 3.0 if is_rg58_profile(params.profile) else max(18.0, 0.018 * length)
    terminal_peak = _response_peak(
        distance, impulse, terminal - terminal_half_width, terminal + 1.4 * terminal_half_width
    )
    reference_peak = max(front_peak, terminal_peak, 2e-4)

    for pos in positions:
        if pos <= 0.0 or pos >= length:
            continue
        attenuation = math.exp(-pos / max(1.35 * length, 40.0))
        if is_rg58_profile(params.profile):
            target_peak = max(
                terminal_peak * rng.uniform(0.050, 0.130),
                front_peak * rng.uniform(0.006, 0.020),
                8.0e-5,
            ) * attenuation
            target_peak = float(np.clip(target_peak, 7.0e-5, 4.5e-3))
            sigma_pos = rng.uniform(0.10, 0.23) + 0.00045 * pos
            separation = rng.uniform(0.28, 0.82) + 0.0010 * pos
            sigma_neg = sigma_pos * rng.uniform(1.05, 1.55)
        else:
            length_gain = 1.0 + 0.65 * min(length / 1800.0, 1.0)
            target_peak = max(
                terminal_peak * rng.uniform(0.090, 0.220) * length_gain,
                front_peak * rng.uniform(0.020, 0.060),
                reference_peak * 0.008,
            ) * attenuation
            target_peak = float(np.clip(target_peak, 2.5e-4, 1.2e-2))
            sigma_pos = float(np.clip(rng.uniform(1.0, 2.2) + 0.0022 * pos, 1.0, 7.0))
            separation = float(np.clip(rng.uniform(4.5, 8.5) + 0.0065 * pos, 5.0, 24.0))
            sigma_neg = sigma_pos * rng.uniform(1.18, 1.75)

        first_center = pos
        second_center = pos + separation
        active = (
            (distance >= first_center - 4.5 * sigma_pos)
            & (distance <= second_center + 4.5 * sigma_neg)
        )
        if int(active.sum()) < 8:
            continue
        positive = np.exp(-0.5 * np.square((distance - first_center) / sigma_pos))
        negative = np.exp(-0.5 * np.square((distance - second_center) / sigma_neg))
        # Equal areas return the step response close to its pre-joint baseline.
        negative_peak_ratio = (sigma_pos / sigma_neg) * rng.uniform(0.88, 1.08)
        shape = target_peak * (positive - negative_peak_ratio * negative)
        # A very small recovery lobe prevents an unnaturally exact two-Gaussian
        # template while remaining clearly bipolar.
        third_sigma = sigma_neg * rng.uniform(1.15, 1.55)
        third_center = second_center + rng.uniform(1.4, 2.2) * sigma_neg
        shape += target_peak * rng.uniform(0.04, 0.11) * np.exp(
            -0.5 * np.square((distance - third_center) / third_sigma)
        )
        shape[~active] = 0.0
...
def build_field_near_end_texture_s11(
    ...
    """Create a stable causal SMA-to-clamp burst directly in S11.

    Real field traces contain many closely spaced front-end echoes rather than
    an arbitrary time-domain waveform.  V2.2 therefore synthesizes a dense,
    irregular train of minimum-phase echoes: a strong 0-15 m cluster followed
    by a weak 15-60 m tail.  Unlike regularized inverse-Hann synthesis, this
    construction stays bounded in frequency and does not depend on clipping.
    """
    del window  # Kept in the API because all front-end builders share it.
    freq_hz = np.asarray(freq_hz, dtype=np.float64)
    ...
    # Broad direct fixture response.  Its delay is deliberately sub-metre so
    # the first displayed sample carries more energy than the 5-15 m shoulder.
    direct = causal_lowpass(
        freq_hz, rng.uniform(260e6, 720e6), rng.uniform(0.65, 1.20)
    )
    correction += rng.choice([-1.0, 1.0]) * rng.uniform(0.95, 1.35) * direct * np.exp(
        -2j * omega * rng.uniform(0.02, 0.28) / velocity
    )

    # Dense, high-bandwidth clamp/lead reverberations.  Random spacing avoids a
    # synthetic comb, while the exponential envelope reproduces the measured
    # decay from 0 m into the 10-18 m knee.
    ring_decay_m = rng.uniform(5.6, 9.2)
    position = rng.uniform(0.20, 0.52)
    echo_index = 0
    while position < knee_m:
        amplitude = (
            rng.uniform(0.64, 1.32)
            * math.exp(-position / ring_decay_m)
            / (1.0 + 0.006 * echo_index)
        )
        lowpass = causal_lowpass(
            freq_hz, rng.uniform(300e6, 1.10e9), rng.uniform(0.35, 0.95)
        )
        correction += rng.choice([-1.0, 1.0]) * amplitude * lowpass * np.exp(
            -2j * omega * position / velocity
        )
        position += rng.uniform(0.15, 0.46) * (1.0 + 0.012 * position)
        echo_index += 1

    # A few damped modes supply the irregular oscillatory skirts observed with
    # alligator-clip fixtures without ringing around remote defects or ends.
    for _ in range(int(rng.randint(2, 5))):
        delay = rng.uniform(0.15, min(4.5, 0.38 * knee_m))
        correction += (
            rng.choice([-1.0, 1.0])
            * rng.uniform(0.10, 0.28)
            * causal_resonant_mode(
                freq_hz, rng.uniform(32e6, 190e6), rng.uniform(1.2, 4.0)
            )
            * np.exp(-2j * omega * delay / velocity)
        )

    # Smooth transition after the dense clamp burst.  V2.1 used isolated,
    # one-sided echoes out to 40-90 m; in Field-short samples they appeared as
    # sparse nuisance peaks.  Keep only a compact, densely alternating release
    # and force it to zero within a few metres of the knee.
    transition_span = rng.uniform(3.5, 6.0) if getattr(cable, "termination", "") == "short" else rng.uniform(5.0, 8.5)
    transition_end = knee_m + transition_span
    position = knee_m + rng.uniform(0.12, 0.35)
    tail_index = 0
    while position < transition_end:
        progress = (position - knee_m) / max(transition_span, 1e-6)
        taper = (1.0 - _smoothstep01(progress)) * math.exp(-2.0 * progress)
        amplitude = rng.uniform(0.035, 0.085) * taper
        lowpass = causal_lowpass(
            freq_hz, rng.uniform(180e6, 620e6), rng.uniform(0.55, 1.20)
        )
        sign = -1.0 if tail_index % 2 else 1.0
        sign *= rng.choice([0.88, 1.0, 1.12])
        correction += sign * amplitude * lowpass * np.exp(
            -2j * omega * position / velocity
        )
        position += rng.uniform(0.20, 0.58)
        tail_index += 1

    # Calibrate the standalone kernel by a robust spectral percentile.  This
    # gives field-family variation while guaranteeing a bounded S11 addition.
    target_p995 = float(np.clip(
        (0.31 + 2.05 * params.fixture_scale) * factor * family_scale,
        0.32,
        0.62,
    ))
    current_p995 = float(np.nanpercentile(np.abs(correction), 99.5))
    if not np.isfinite(current_p995) or current_p995 <= 1e-12:
        return np.zeros_like(freq_hz, dtype=np.complex128)
    correction *= target_p995 / current_p995
    return correction
...
def apply_moisture_response_s11(
    ...
    for region in regions:
        ...
        slope_delta = float(np.clip(target_post_slope - post_slope, -4.0e-13, 4.0e-13))
        ...
        if safe_end > anchor + 60.0 and abs(slope_delta) > 2e-15:
            ...
            trend = np.clip(trend, -7.0e-11, 7.0e-11)
...
def generate_band(
    ...
    # V2.2 invariant: these are the only response calculations.  No distance-
