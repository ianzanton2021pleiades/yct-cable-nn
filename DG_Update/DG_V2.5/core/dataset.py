"""PyTorch dataset loader for DG V2.5 outputs.

The loader always recomputes the distance responses from the selected S11 CSV
with the shared Client IFFT.  DG V2.5 guarantees that these responses are the same
...
class CableDefectDataset(Dataset):
    """Load one DG V2.5 split and return aligned input/label tensors.
    ...
    Args:
        ...
        band: ``1GHz`` or ``200MHz``.
        d_max, dd: fixed distance grid; defaults match the V2.5 label grid.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(label) != self.n_grid:
            raise ValueError(
                ...
                "Use d_max=2500 and dd=0.25 for the standard DG V2.5 dataset."
            )
