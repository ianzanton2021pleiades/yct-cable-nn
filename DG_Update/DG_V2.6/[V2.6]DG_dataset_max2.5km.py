"""
[V2.6]DG_dataset_max2.5km.py - DirtyGenerator for <=2.5 km cable S11 datasets.
...
def defect_type_policy(profile: str, total_length: float) -> dict:
    ...
    return {
        ...
        # The first four entries reproduce the original V2.6 families.  The
...
def apply_field_baseline_texture_s11(
    ...
    The V2.6 bridge process was built from linearly interpolated random knots.
    Differentiating it created flat-sided, saw-tooth pulses and the same corners
    accumulated into a jagged step response.  V2.6 instead synthesizes a
...
def shape_clean_terminal_s11(
    ...
    Separating removal from insertion is the key V2.6 architectural fix: S11
...
def build_single_terminal_branch_s11(
    ...
    if is_rg58_profile(profile):
        ...
        # V2.6 reused a 3-6 m low-pass scale here and consequently produced an
...
def apply_field_spectral_family_calibration(
    ...
    V2.6 trials.  The compact front-end kernel itself now determines the Field
...
def apply_field_background_clutter_s11(
    ...
    same amount as a distributed moisture defect.  V2.6 draws a bridge process
...
def apply_moisture_response_s11(
    ...
    for region in regions:
        ...
        slope_cap = 8.0e-15 if defect_type == 'moisture_distributed' else 1.4e-14
        slope = float(np.clip(raw_slope, -slope_cap, slope_cap))
...
def generate_band(
    ...
    # V2.6 invariant: these are the only response calculations.  No distance-
