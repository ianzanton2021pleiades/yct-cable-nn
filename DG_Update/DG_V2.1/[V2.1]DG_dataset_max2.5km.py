"""
[V2.1]DG_dataset_max2.5km.py - DirtyGenerator for <=2.5 km cable S11 datasets.
...
def assess_template_quality(path: Path, profile: str) -> float:
    ...
    V2.1 never transfers the template's coherent phase, so this score only needs
...
def apply_measured_template_shape(
    ...
    defect in that trace therefore became an unlabeled synthetic peak.  V2.1
...
def step_shape_to_s11_kernel(
    ...
    It lets V2.1 encode calibrated fixture/background/moisture morphology in
...
def build_rg58_near_end_texture_s11(
    ...
    front_gain = 1.0 + rng.uniform(0.70, 1.35) * np.exp(
        -np.power(d_local / max(boost_span, 1e-6), rng.uniform(1.15, 1.55))
    )
    target_rms = params.fixture_scale * rng.uniform(0.030, 0.062)
    target[mask] = target_rms * base_envelope * front_gain * texture
    target[0] += (
        rng.choice([-1.0, 1.0])
...
def build_field_near_end_texture_s11(
    ...
    an arbitrary time-domain waveform.  V2.1 therefore synthesizes a dense,
    ...
    tail_extent_m = float(np.clip(34.0 + 0.024 * length, 42.0, 94.0))
    ...
    # Far weaker, lower-bandwidth lead/coupling echoes after the main cluster.
    position = knee_m + rng.uniform(0.45, 1.8)
    tail_decay_m = rng.uniform(16.0, 34.0)
    while position < tail_extent_m:
        amplitude = rng.uniform(0.018, 0.050) * math.exp(
            -(position - knee_m) / tail_decay_m
        )
        lowpass = causal_lowpass(
            freq_hz, rng.uniform(90e6, 360e6), rng.uniform(0.75, 1.55)
        )
        correction += rng.choice([-1.0, 1.0]) * amplitude * lowpass * np.exp(
            -2j * omega * position / velocity
        )
        position += rng.uniform(0.75, 2.9)
...
def generate_band(
    ...
    # V2.1 invariant: these are the only response calculations.  No distance-
...
def weighted_template_choice(
    ...
    broadband attenuation statistics.  V2.1 therefore performs a cheap
