...
def apply_bipolar_joint_model_s11(
    ...
    for pos in positions:
        ...
        path_loss = _moisture_path_transmission(cable, pos)
...
def build_field_near_end_texture_s11(
    ...
    """Create a coherent SMA-to-clamp field fixture spectrum.

    Real field S11 is usually dominated by a few short-delay fixture modes with
    a smooth frequency envelope; the kilometre-scale cable return appears as a
    much smaller fast ripple.  V2.2 used dozens of comparable random echoes,
    which produced an over-random real/imaginary trace.  V2.3 uses one dominant
    6-15 m mode, a few weaker neighbouring modes, and only a low-amplitude dense
    echo cloud for the near-end IFFT burst.
    """
    del window
    freq_hz = np.asarray(freq_hz, dtype=np.float64)
    ...
    main_delay = float(np.clip(
        rng.uniform(6.5, 13.5) + 0.0012 * min(length, 1800.0),
        6.0,
        15.5,
    ))
    main_corner = rng.uniform(120e6, 260e6)
    main_order = rng.uniform(0.90, 1.35)
    main_amp = rng.uniform(0.30, 0.50) * factor * family_scale
    main_phase = rng.uniform(-math.pi, math.pi)
    correction += main_amp * np.exp(1j * main_phase) * causal_lowpass(
        freq_hz, main_corner, main_order
    ) * np.exp(-2j * omega * main_delay / velocity)

    # A broad almost-direct path sets the low-frequency complex offset and keeps
    # the very first samples energetic without replacing the dominant carrier.
    correction += (
        rng.uniform(0.035, 0.085)
        * factor
        * family_scale
        * np.exp(1j * rng.uniform(-math.pi, math.pi))
        * causal_lowpass(freq_hz, rng.uniform(420e6, 900e6), rng.uniform(0.55, 0.95))
        * np.exp(-2j * omega * rng.uniform(0.05, 0.65) / velocity)
    )

    # Two or three coherent neighbouring paths create the measured slow beating
    # and envelope undulation while preserving a smooth Re/Im trajectory.
    for _ in range(int(rng.randint(2, 4))):
        delay = float(np.clip(main_delay + rng.uniform(-4.2, 5.5), 1.2, 19.0))
        amp = main_amp * rng.uniform(0.045, 0.14)
        corner = main_corner * rng.uniform(0.72, 1.55)
        order = rng.uniform(0.55, 1.18)
        correction += (
            amp
            * np.exp(1j * rng.uniform(-math.pi, math.pi))
            * causal_lowpass(freq_hz, corner, order)
            * np.exp(-2j * omega * delay / velocity)
        )

    # Broad resonances model clamp/contact impedance without turning the whole
    # band into a random walk.
    for _ in range(int(rng.randint(2, 5))):
        correction += (
            rng.uniform(0.008, 0.030)
            * factor
            * family_scale
            * np.exp(1j * rng.uniform(-math.pi, math.pi))
            * causal_resonant_mode(
                freq_hz, rng.uniform(45e6, 420e6), rng.uniform(0.8, 3.6)
            )
            * np.exp(-2j * omega * rng.uniform(0.2, 4.0) / velocity)
        )

    # Low-level dense echoes retain the accepted compact near-end ringing.  Their
    # total spectral energy is deliberately far below the coherent main mode.
    knee_m = float(np.clip(9.5 + 0.0055 * length, 10.5, 18.0))
    position = rng.uniform(0.18, 0.42)
    echo_index = 0
    while position < knee_m:
        amplitude = rng.uniform(0.0015, 0.0055) * math.exp(-position / rng.uniform(4.8, 8.5))
        correction += (
            rng.choice([-1.0, 1.0])
            * amplitude
            * factor
            * family_scale
            * causal_lowpass(freq_hz, rng.uniform(380e6, 1.1e9), rng.uniform(0.35, 0.85))
            * np.exp(-2j * omega * position / velocity)
        )
        position += rng.uniform(0.18, 0.46) * (1.0 + 0.010 * position)
        echo_index += 1
    ...
    target_low = float(np.clip(
        rng.uniform(0.32, 0.50) * factor * family_scale,
        0.26,
        0.58,
    ))
    if np.isfinite(current_low) and current_low > 1e-12:
        correction *= float(np.clip(target_low / current_low, 0.55, 1.55))
    return correction
...
def _field_cable_visibility_transfer(
    ...
    low_gain = rng.uniform(0.025, 0.080)
    floor = rng.uniform(0.0010, 0.0065)
    order = rng.uniform(0.78, 1.12)
...
def apply_moisture_response_s11(
    ...
    for region in regions:
        ...
        if defect_type == "moisture_local":
            drop = float(np.clip(
                response_scale * (0.25 + 0.38 * severity) * shape_scale,
                2.8e-11,
                1.10e-10,
            ))
            ...
            drop = float(np.clip(
                response_scale * (0.52 + 0.62 * severity) * length_gain * shape_scale,
                7.0e-11,
                2.35e-10,
            ))
