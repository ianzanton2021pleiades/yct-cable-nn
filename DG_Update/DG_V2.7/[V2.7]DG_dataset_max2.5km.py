"""
[V2.7]DG_dataset_max2.5km.py - DirtyGenerator for <=2.5 km cable S11 datasets.
...
def defect_type_policy(profile: str, total_length: float) -> dict:
    ...
    return {
        ...
        # The first four entries reproduce the original V2.7 families.  The
...
def append_gradual_defect_segments(
    ...
    if defect_type == "aging":
        ...
        # The macroscopic wet-section impedance drop is imposed by the final
        # continuous step model.  Keeping the graded physical backbone almost
        # impedance-matched prevents every numerical taper segment from
        # appearing as an individual negative pulse in the IFFT.
        z0_weight = 0.0
...
def _replace_local_impulse_window_s11(
    out: np.ndarray,
    freq_hz: np.ndarray,
    cable: CableSample,
    ...
    """Replace one local response instead of stacking another reflection pair.

    A physical RLGC discontinuity already creates a boundary response.  Adding
    a second analytic visibility kernel can therefore produce an unlabeled
    precursor several metres before the defect.  This routine performs a local
    alternating projection in the *same* Client IFFT grid: the existing window
    is flattened to a smooth local baseline plus the requested event, while a
    zero-area closure preserves the step level on both sides.
    """
    result = np.asarray(out, dtype=np.complex128).copy()
    target_event = np.asarray(target_event, dtype=np.float64)
    if target_event.ndim != 1:
        return result
    for iteration in range(max(int(iterations), 1)):
        try:
            ...
            return result
        if len(distance) != len(target_event):
            return result
        current = np.real(impulse)
        active = (distance >= float(lo_m)) & (distance <= float(hi_m))
        idx = np.flatnonzero(active)
        if len(idx) < 24:
            return result

        span = max(float(hi_m) - float(lo_m), 1.0)
        pre = (distance >= float(lo_m) - max(10.0, 0.35 * span)) & (distance < float(lo_m) - 0.04 * span)
        post = (distance > float(hi_m) + 0.04 * span) & (distance <= float(hi_m) + max(10.0, 0.35 * span))
        left_level = float(np.nanmedian(current[pre])) if int(pre.sum()) >= 6 else float(current[idx[0]])
        right_level = float(np.nanmedian(current[post])) if int(post.sum()) >= 6 else float(current[idx[-1]])
        u = np.linspace(0.0, 1.0, len(idx), dtype=np.float64)
        baseline = left_level + (right_level - left_level) * _smoothstep01(u)
        desired = baseline + target_event[idx]

        taper = np.ones(len(idx), dtype=np.float64)
        edge = max(8, int(round(0.14 * len(idx))))
        edge = min(edge, max(3, len(idx) // 3))
        taper[:edge] = _smoothstep01(np.linspace(0.0, 1.0, edge))
        taper[-edge:] = _smoothstep01(np.linspace(1.0, 0.0, edge))
        residual = np.zeros_like(current)
        residual[idx] = (desired - current[idx]) * taper

        # Keep the integrated step unchanged outside the edit window.  A broad
        # smooth closure avoids a compensating needle at either window edge.
        closure = np.exp(-0.5 * np.square((u - 0.5) / 0.30)) * taper
        area = float(np.sum(residual[idx]))
        residual[idx] -= area * closure / max(float(np.sum(closure)), 1e-12)

        gain = 0.94 if iteration == 0 else 0.78
        kernel = localized_time_kernel_to_s11(freq_hz, gain * residual, window=window)
        kernel = _scale_additive_kernel_to_budget(
            result, kernel, percentile_limit=1.12, max_limit=1.23
        )
        result += kernel
        mag = np.abs(result)
        over = mag > 1.22
        if np.any(over):
            result[over] *= 1.22 / mag[over]
    return result
...
def apply_field_defect_visibility_s11(
    ...
    for defect in cable.defect_info:
        ...
        if defect_type == "short":
            ...
            defect_len = max(end_m - start_m, 0.8)
            width1 = float(np.clip(0.13 * defect_len + rng.uniform(0.16, 0.48), 0.55, 1.75))
            width2 = float(np.clip(width1 * rng.uniform(1.25, 1.65), 0.75, 2.55))
            entry_pos = start_m + float(np.clip(0.20 * defect_len, 0.55 * width1, 0.36 * defect_len))
            exit_pos = end_m - float(np.clip(0.18 * defect_len, 0.45 * width2, 0.34 * defect_len))
            if exit_pos <= entry_pos + 0.35 * width1:
                entry_pos = start_m + 0.28 * defect_len
                exit_pos = end_m - 0.24 * defect_len
            sign = 1.0 if float(defect.get("z0", cable.z_ref)) >= float(cable.z_ref) else -1.0
            ratio = rng.uniform(0.48, 0.72)
            target_event = target_peak * sign * (
                np.exp(-0.5 * np.square((distance - entry_pos) / max(width1, 1e-6)))
                - ratio * np.exp(-0.5 * np.square((distance - exit_pos) / max(width2, 1e-6)))
            )
            lo = start_m - max(16.0, 7.0 * width2)
            hi = end_m + max(16.0, 7.0 * width2)
            target_event[(distance < start_m - 2.2 * width1) | (distance > end_m + 3.0 * width2)] = 0.0
            result = _replace_local_impulse_window_s11(
                result, freq_hz, cable, target_event, lo, hi,
                window=window, iterations=5,
            )
            continue
        else:
            if defect_type == "aging":
...
def finalize_field_short_windows_s11(
    out: np.ndarray,
    freq_hz: np.ndarray,
    cable: CableSample,
    rng: np.random.RandomState,
    ...
    """Final local cleanup for Field short defects after all global shaping.

    Template envelopes, nominal-slope correction and terminal headroom are
    applied at different stages.  This final pass prevents any of them from
    re-exposing the physical RLGC precursor that the local defect replacement
    removed earlier.
    """
    if params.profile != "field":
        return out
    ...
    for defect in cable.defect_info:
        ...
        defect_len = max(end - start, 0.8)
        main = (distance >= start - 1.0) & (distance <= end + 2.0)
        if int(main.sum()) < 8:
            continue
        peak = float(np.max(np.abs(np.real(impulse[main]))))
        if peak <= 1e-6:
            continue
        width1 = float(np.clip(0.13 * defect_len + 0.30, 0.58, 1.70))
        width2 = float(np.clip(1.45 * width1, 0.82, 2.50))
        entry = start + float(np.clip(0.22 * defect_len, 0.58 * width1, 0.36 * defect_len))
        exit_ = end - float(np.clip(0.19 * defect_len, 0.48 * width2, 0.34 * defect_len))
        if exit_ <= entry + 0.35 * width1:
            entry = start + 0.28 * defect_len
            exit_ = end - 0.24 * defect_len
        sign = 1.0 if float(defect.get("z0", cable.z_ref)) >= float(cable.z_ref) else -1.0
        ratio = float(np.clip(0.56 + 0.06 * (defect_len / 6.0 - 1.0), 0.50, 0.68))
        target = peak * sign * (
            np.exp(-0.5 * np.square((distance - entry) / max(width1, 1e-6)))
            - ratio * np.exp(-0.5 * np.square((distance - exit_) / max(width2, 1e-6)))
        )
        target[(distance < start - 2.2 * width1) | (distance > end + 3.0 * width2)] = 0.0
        result = _replace_local_impulse_window_s11(
            result,
            freq_hz,
            cable,
            target,
            start - max(18.0, 7.5 * width2),
            end + max(18.0, 7.5 * width2),
            window=window,
            iterations=4,
        )
    return result
...
def apply_field_baseline_texture_s11(
    ...
    """Add a continuous Field micro-mismatch baseline with bounded integral.

    The V2.7 bridge process was built from linearly interpolated random knots.
    Differentiating it created flat-sided, saw-tooth pulses and the same corners
    accumulated into a jagged step response.  V2.7 instead synthesizes a
    differentiable correlated process directly in distance: two Gaussian
    scales are high-pass combined, smoothly tapered, and closed with a broad
    zero-area correction.  The impulse therefore remains continuously varying,
    while its cumulative step contribution is strictly bounded.
    """
    if params.profile != "field":
        return out
    result = np.asarray(out, dtype=np.complex128).copy()
    ...
    length = max(float(cable.total_length), 1.0)
    terminal = float(effective_terminal_phase_length_m(cable))
    body_start = float(np.clip(16.0 + 0.0035 * length, 17.0, 27.0))
    body_end = min(
        terminal - max(24.0, 0.020 * length),
        float(distance[-1]) * 0.94,
    )
    active = (distance >= body_start) & (distance <= body_end)
    if int(active.sum()) < 160:
        return result

    dx = max(float(np.median(np.diff(distance[: min(len(distance), 4096)]))), 1e-6)
    n = len(distance)

    def gaussian_correlated(corr_m: float) -> np.ndarray:
        sigma = float(np.clip(corr_m / dx, 1.2, 420.0))
        radius = max(5, int(math.ceil(3.8 * sigma)))
        xk = np.arange(-radius, radius + 1, dtype=np.float64)
        kernel = np.exp(-0.5 * np.square(xk / sigma))
        kernel /= max(float(np.sqrt(np.sum(kernel * kernel))), 1e-12)
        return np.convolve(rng.normal(size=n), kernel, mode="same")

    # Difference-of-scales removes the near-DC component that would otherwise
    # integrate into a random walk, but unlike a hard high-pass it remains
    # smooth in both value and slope.
    fast = gaussian_correlated(rng.uniform(0.55, 1.35))
    medium = gaussian_correlated(rng.uniform(2.8, 7.5))
    broad = gaussian_correlated(rng.uniform(22.0, 65.0))
    texture = 0.78 * fast + 0.32 * medium - 0.24 * broad
    texture -= gaussian_correlated(rng.uniform(55.0, 120.0)) * 0.10
    texture -= float(np.nanmedian(texture[active]))

    # Distance attenuation follows the measured field archive: clear but small
    # early body texture, then a slow decay to a non-zero far-end floor.
    decay_m = rng.uniform(0.24, 0.42) * length + rng.uniform(35.0, 90.0)
    remote_floor = rng.uniform(0.15, 0.30)
    envelope = remote_floor + (1.0 - remote_floor) * np.exp(-distance / max(decay_m, 1.0))

    entrance_span = max(8.0, 0.016 * length)
    exit_span = max(18.0, 0.024 * length)
    gate = _smoothstep01((distance - body_start) / entrance_span)
    gate *= 1.0 - _smoothstep01((distance - (body_end - exit_span)) / exit_span)

    # Wet cable attenuates the background only gradually; no hard amplitude
    # edge is introduced at a moisture boundary.
    wet_scale = np.ones_like(distance)
    for region in _moisture_regions(cable):
        start = float(region["start"])
        end = float(region["end"])
        severity = float(np.clip(region.get("severity", 0.62), 0.30, 0.90))
        transition = _smoothstep01((distance - start) / max(end - start, 1.0))
        retained = 0.58 + 0.24 * (1.0 - severity)
        wet_scale *= 1.0 - (1.0 - retained) * transition

    target = texture * envelope * gate * wet_scale
    target[~active] = 0.0

    # Soft event notches prevent the baseline from masking labels without
    # producing flat gaps or corners.
    events = [float(p) for p in getattr(cable, "joint_positions", [])]
    events.extend(float(d["position"]) for d in cable.defect_info)
    for event in events:
        width = float(np.clip(0.0035 * length, 2.2, 9.0))
        target *= 1.0 - 0.50 * np.exp(-0.5 * np.square((distance - event) / width))

    # Enforce zero total area with one broad smooth closure, not a linear or
    # piecewise correction.  This keeps the final step response from drifting.
    idx = np.flatnonzero(active)
    local_x = np.linspace(-1.0, 1.0, len(idx))
    closure = np.exp(-0.5 * np.square(local_x / 0.42))
    area = float(np.sum(target[idx]))
    target[idx] -= area * closure / max(float(np.sum(closure)), 1e-12)

    front_peak = _response_peak(distance, impulse, 0.0, min(24.0, 0.07 * length + 8.0))
    terminal_peak = _response_peak(
        distance,
        impulse,
        terminal - max(16.0, 0.012 * length),
        terminal + max(28.0, 0.020 * length),
    )
    desired_rms = float(np.clip(
        max(
            front_peak * rng.uniform(0.0014, 0.0036),
            terminal_peak * rng.uniform(0.005, 0.012),
            rng.uniform(4.5e-6, 1.15e-5),
        ),
        4.0e-6,
        3.8e-5,
    ))
    current_rms = _rms(target[active])
    if current_rms <= 1e-12:
        return result
    target *= desired_rms / current_rms

    # Bound the step excursion produced by this nuisance component.  Scaling
    # is evaluated in the exact Client time grid before the inverse mapping.
    _, _, dt = _client_ifft_axes(freq_hz, cable.epsr)
    cumulative = np.cumsum(target) * dt
    body_span = float(np.nanpercentile(cumulative[active], 95) - np.nanpercentile(cumulative[active], 5))
    span_limit = rng.uniform(0.9e-11, 2.3e-11) * (0.90 + 0.12 * min(length / 1800.0, 1.0))
    if np.isfinite(body_span) and body_span > span_limit and body_span > 1e-20:
        target *= span_limit / body_span

    kernel_s11 = _impulse_kernel_from_shape(
        freq_hz, cable, target, active, window=window
    )
    kernel_s11 = _scale_additive_kernel_to_budget(
        result, kernel_s11, percentile_limit=1.08, max_limit=1.18
    )
    return result + kernel_s11
...
def shape_clean_terminal_s11(
    ...
    Separating removal from insertion is the key V2.7 architectural fix: S11
...
def build_single_terminal_branch_s11(
    ...
    if is_rg58_profile(profile):
        # RG58 open/short ends are compact: a high narrow crest followed by a
        # short several-metre decay, not the broad field-cable terminal tail.
        # V2.7 reused a 3-6 m low-pass scale here and consequently produced an
        # over-wide, low peak on 100-200 m coax samples.  The two scales below
        # retain one causal end reflection while moving substantially more
        # energy into the fast branch.
        fast_m = float(np.clip(rng.uniform(0.10, 0.19) + 0.00018 * length, 0.11, 0.25))
        slow_m = float(np.clip(rng.uniform(0.78, 1.45) + 0.0018 * length, 0.82, 1.85))
        dc_amp = rng.uniform(0.060, 0.105)
        fast_fraction = rng.uniform(0.44, 0.62)
        fast_order = rng.uniform(0.88, 1.05)
        slow_order = rng.uniform(1.02, 1.22)
    else:
        if length < 500.0:
            fast_m = rng.uniform(0.24, 0.40) + 0.00042 * length
            slow_m = rng.uniform(8.5, 14.0) + 0.010 * length
            dc_amp = rng.uniform(0.84, 0.96)
            fast_fraction = rng.uniform(0.58, 0.69)
        elif length < 1500.0:
            fast_m = rng.uniform(0.72, 1.28) + 0.00050 * length
            slow_m = rng.uniform(15.0, 25.0) + 0.008 * length
            dc_amp = rng.uniform(0.62, 0.84)
            fast_fraction = rng.uniform(0.42, 0.54)
        else:
            fast_m = rng.uniform(1.10, 2.05) + 0.00038 * length
            slow_m = rng.uniform(27.0, 43.0) + 0.006 * length
            dc_amp = rng.uniform(0.48, 0.72)
            fast_fraction = rng.uniform(0.36, 0.48)
...
def build_field_near_end_texture_s11(
    ...
    """Build a compact, broadband Field clamp response on the Client grid.

    Field sweeps in the archive are not a single chirped packet like RG58.
    Their real/imaginary traces contain irregular but adjacent-frequency
    coherent oscillations across the whole band, with much larger energy below
    roughly 200 MHz.  This is generated from a *compact correlated random
    fixture waveform* in the first 14--20 m.  A compact distance support keeps
    IFFT/step causality exact; two correlation scales and broad contact lobes
    yield the measured packet-to-packet variability without importing any
    remote reflection from a template.
    """
    del calibration_template
    freq_hz = np.asarray(freq_hz, dtype=np.float64)
    _, distance, _ = _client_ifft_axes(freq_hz, cable.epsr)
    length = max(float(cable.total_length), 1.0)
    extent = float(np.clip(14.0 + 0.0032 * length, 14.5, 20.0))
    active = distance <= extent
    if int(active.sum()) < 80:
        return np.zeros_like(freq_hz, dtype=np.complex128)

    d = distance[active]
    dx = max(float(np.median(np.diff(d))), 1e-6)
    family = float(params.field_near_end_scale)

    def correlated(corr_m: float) -> np.ndarray:
        sigma = float(np.clip(corr_m / dx, 1.0, 80.0))
        radius = min(max(4, int(math.ceil(3.8 * sigma))), max(4, (len(d) - 1) // 2))
        xk = np.arange(-radius, radius + 1, dtype=np.float64)
        kernel = np.exp(-0.5 * np.square(xk / sigma))
        kernel /= max(float(np.sqrt(np.sum(kernel * kernel))), 1e-12)
        return np.convolve(rng.normal(size=len(d)), kernel, mode="same")

    # Dense fine structure plus a slower return-path mode.  Using a correlated
    # stochastic cluster (rather than one chirp) avoids the artificial giant
    # spectral packet around one frequency while retaining smooth Re/Im traces.
    raw = rng.normal(size=len(d))
    fine = correlated(rng.uniform(0.055, 0.12))
    medium = correlated(rng.uniform(0.30, 0.75))
    broad = correlated(rng.uniform(1.8, 3.8))
    raw /= max(float(np.std(raw)), 1e-12)
    fine /= max(float(np.std(fine)), 1e-12)
    medium /= max(float(np.std(medium)), 1e-12)
    broad /= max(float(np.std(broad)), 1e-12)
    cluster = 0.22 * raw + 0.50 * fine + 0.24 * medium + 0.05 * broad

    # Field clamp ringing persists farther than the direct SMA/BNC response.
    # A slight two-stage decay produces the measured strong first few metres and
    # lower-amplitude tail without isolated sparse peaks.
    envelope = (
        0.78 * np.exp(-d / rng.uniform(3.6, 6.2))
        + 0.22 * np.exp(-d / rng.uniform(8.0, 13.0))
    )
    cluster *= envelope

    slow_period = rng.uniform(2.2, 4.8)
    slow_phase = rng.uniform(-math.pi, math.pi)
    cluster += rng.uniform(0.08, 0.18) * np.sin(
        2.0 * math.pi * d / slow_period + slow_phase
    ) * np.exp(-d / rng.uniform(7.0, 12.5))

    # Broad contact/return-path lobes distinguish Field clamps from RG58 while
    # remaining fully inside the fixture region.
    center = rng.uniform(3.8, 8.0)
    sigma = rng.uniform(0.55, 1.25)
    cluster += rng.uniform(0.10, 0.24) * np.exp(-0.5 * np.square((d - center) / sigma))
    cluster -= rng.uniform(0.08, 0.20) * np.exp(
        -0.5 * np.square(
            (d - (center + rng.uniform(0.9, 2.4)))
            / (sigma * rng.uniform(1.25, 2.0))
        )
    )

    # Smoothly start and release the compact fixture waveform.  The broad
    # zero-area closure prevents any fixture DC leakage from becoming a cable-
    # length step offset.
    entrance = _smoothstep01(d / max(rng.uniform(0.12, 0.32), 1e-6))
    release = 1.0 - _smoothstep01((d - 0.80 * extent) / max(0.20 * extent, 1e-6))
    cluster *= entrance * release
    closure = np.exp(-0.5 * np.square((d - 0.60 * extent) / max(0.19 * extent, 0.9)))
    cluster -= float(np.sum(cluster)) * closure / max(float(np.sum(closure)), 1e-12)

    # Calibrate in the time domain, where the requested fixture prominence is
    # physically interpretable and independent of frequency-grid density.
    peak = max(float(np.max(np.abs(cluster))), 1e-12)
    cluster *= rng.uniform(0.045, 0.082) * family / peak
    target = np.zeros_like(distance, dtype=np.float64)
    target[active] = cluster

    kernel = _impulse_kernel_from_shape(freq_hz, cable, target, active, window=window)

    # Measured Field spectra are low-frequency dominant but retain a non-zero
    # high-frequency floor.  The causal roll-off preserves exact compact IFFT
    # support, unlike an arbitrary magnitude-only spectral envelope.
    floor = rng.uniform(0.04, 0.09)
    roll = floor + (1.0 - floor) * causal_lowpass(
        freq_hz, rng.uniform(125e6, 195e6), rng.uniform(1.00, 1.34)
    )
    shaped = kernel * roll

    # Broad, low-Q clamp/ground-return modes keep the 250--600 MHz region
    # populated, as in the field archive, without the narrow high-Q comb that
    # characterises the direct RG58 connector.  Their delays stay inside the
    # fixture zone, so they only enrich the same near-end IFFT cluster.
    omega = 2.0 * math.pi * freq_hz
    velocity = 299_792_458.0 / math.sqrt(max(float(cable.epsr), 1.1))
    for _ in range(int(rng.randint(3, 6))):
        f0 = rng.uniform(245e6, 520e6)
        q = rng.uniform(1.35, 3.2)
        delay_m = rng.uniform(0.6, min(7.5, 0.46 * extent))
        amp = rng.uniform(0.050, 0.115) * family
        phase0 = rng.uniform(-math.pi, math.pi)
        shaped += amp * np.exp(1j * phase0) * causal_resonant_mode(
            freq_hz, f0, q
        ) * np.exp(-2j * omega * delay_m / velocity)
    return shaped
...
def _field_cable_visibility_transfer(
    ...
    high-frequency reduction is applied; unlike V2.5, the cable is never reduced
...
def apply_field_spectral_family_calibration(
    out: np.ndarray,
    freq_hz: np.ndarray,
    rng: np.random.RandomState,
    template_s11: np.ndarray | None,
) -> np.ndarray:
    """Set Field broadband energy without importing a non-causal envelope.

    Only one global robust level is transferred from a measured template.  A
    frequency-varying arbitrary gain was the source of long step ramps in early
    V2.7 trials.  The compact front-end kernel itself now determines the Field
    real/imaginary morphology and high-frequency roll-off.
    """
    del freq_hz
    result = np.asarray(out, dtype=np.complex128).copy()
    current = max(float(np.nanpercentile(np.abs(result), 75)), 1e-6)
    if template_s11 is not None and len(template_s11) == len(result):
        measured = float(np.nanpercentile(np.abs(template_s11), 75))
        target = measured * rng.uniform(0.68, 0.86)
    else:
        target = rng.uniform(0.34, 0.52)
    gain = float(np.clip(target / current, 0.72, 1.85))
    result *= gain
    mag = np.abs(result)
    over = mag > 1.04
    if np.any(over):
        result[over] *= 1.04 / mag[over]
    return result
...
def apply_field_background_clutter_s11(
    ...
    same amount as a distributed moisture defect.  V2.7 draws a bridge process
...
def apply_field_nominal_step_slope_s11(
    out: np.ndarray,
    freq_hz: np.ndarray,
    cable: CableSample,
    rng: np.random.RandomState,
    ...
    """Enforce a smooth, shallow downward Field step baseline.

    A low-order trend is replaced iteratively in the exact Client IFFT grid.
    Local joints, defects and continuous micro-texture remain in the residual;
    only the long-scale baseline is corrected.  Holding the correction constant
    before the terminal prevents an artificial end-adjacent pulse.
    """
    if params.profile != "field":
        return out
    result = np.asarray(out, dtype=np.complex128).copy()
    length = max(float(cable.total_length), 1.0)
    terminal = float(effective_terminal_phase_length_m(cable))
    start_m = float(np.clip(22.0 + 0.006 * length, 24.0, 40.0))
    end_m = terminal - max(55.0, 0.038 * length)
    target_slope = -rng.uniform(0.75e-14, 2.10e-14)
    target_slope *= float(np.clip((length / 1200.0) ** 0.12, 0.86, 1.14))
    curvature = rng.uniform(0.04, 0.10)

    for iteration in range(4):
        try:
            distance, _, step, _ = s11_to_responses(
                freq_hz, result, epsr=cable.epsr, window=window
            )
        except Exception:
            return result
        end = min(end_m, float(distance[-1]) - max(42.0, 0.028 * length))
        if end <= start_m + 100.0:
            return result
        active = (distance >= start_m) & (distance <= end)
        idx = np.flatnonzero(active)
        if len(idx) < 100:
            return result
        dd = max(float(np.median(np.diff(distance[: min(len(distance), 4096)]))), 1e-6)
        smooth_m = float(np.clip(0.070 * length, 65.0, 150.0))
        smooth_n = int(np.clip(round(smooth_m / dd), 81, 1801))
        if smooth_n % 2 == 0:
            smooth_n += 1
        smooth_step = smooth_array(step, smooth_n)

        anchor_band = (distance >= start_m) & (distance <= start_m + max(24.0, 0.025 * length))
        anchor_level = float(np.nanmedian(smooth_step[anchor_band])) if int(anchor_band.sum()) else float(smooth_step[idx[0]])
        span = max(end - start_m, 1.0)
        u = np.clip((distance - start_m) / span, 0.0, 1.0)
        target = anchor_level + target_slope * (distance - start_m)
        target -= curvature * abs(target_slope) * span * np.square(u)

        delta = np.zeros_like(distance, dtype=np.float64)
        delta[idx] = target[idx] - smooth_step[idx]
        enter_span = max(24.0, 0.035 * span)
        enter = _smoothstep01((distance - start_m) / enter_span)
        delta[idx] *= enter[idx]
        last = float(delta[idx[-1]])
        delta[distance > end] = last
        try:
            correction = step_shape_to_s11_kernel(
                freq_hz, cable.epsr, delta, window=window
            )
        except Exception:
            return result
        correction *= 0.94 if iteration == 0 else 0.78
        correction = _scale_additive_kernel_to_budget(
            result, correction, percentile_limit=1.12, max_limit=1.22
        )
        result += correction

    # Final slope-only calibration removes seed-dependent residual trends from
    # the physical carrier while leaving all local events untouched.
    for _ in range(2):
        try:
            distance, _, step, _ = s11_to_responses(
                freq_hz, result, epsr=cable.epsr, window=window
            ...
            break
        body = (distance >= max(0.10 * length, start_m + 35.0)) & (
            distance <= min(0.86 * length, terminal - max(90.0, 0.055 * length))
        )
        if int(body.sum()) < 80:
            break
        dd = max(float(np.median(np.diff(distance[: min(len(distance), 4096)]))), 1e-6)
        smooth_n = int(np.clip(round(max(75.0, 0.055 * length) / dd), 81, 1601))
        if smooth_n % 2 == 0:
            smooth_n += 1
        smooth_step = smooth_array(step, smooth_n)
        current_slope = float(np.polyfit(distance[body], smooth_step[body], 1)[0])
        slope_error = float(target_slope - current_slope)
        if abs(slope_error) <= 2.5e-15:
            break
        end = min(end_m, float(distance[-1]) - max(42.0, 0.028 * length))
        span = max(end - start_m, 1.0)
        u = np.clip((distance - start_m) / span, 0.0, 1.0)
        progress = u.copy()
        progress *= _smoothstep01(u / 0.06)
        correction_step = slope_error * span * progress
        correction_step[distance < start_m] = 0.0
        correction_step[distance > end] = slope_error * span
        try:
            kernel = step_shape_to_s11_kernel(
                freq_hz, cable.epsr, correction_step, window=window
            )
        except Exception:
            break
        kernel = _scale_additive_kernel_to_budget(
            result, kernel, percentile_limit=1.12, max_limit=1.22
        )
        result += kernel
    return result
...
def apply_moisture_response_s11(
    ...
    random walks and the unrealistically abrupt wet-section drop of V2.5.
    """
    ...
    for region in regions:
        ...
        # Anchor the healthy baseline to the robust pre-wet median.  Fitting an
        # intercept at the wet boundary allowed a rising fixture/terminal trend
        # to lift the entire target above the healthy section.  Only a very
        # small capped slope is retained; the visible descent must therefore
        # come from the wet interval itself.
        raw_slope, _ = np.polyfit(xpre - float(np.median(xpre)), ypre, 1)
        # Real Field baselines usually carry a small intrinsic downward slope.
        # Preserve that trend while rejecting positive fixture drift; the wet
        # interval must remain the dominant additional descent.
        negative_cap = 3.0e-14 if defect_type == 'moisture_distributed' else 3.8e-14
        positive_cap = 5.0e-15 if defect_type == 'moisture_distributed' else 8.0e-15
        slope = float(np.clip(raw_slope, -negative_cap, positive_cap))
        anchor_x = float(np.median(xpre))
        anchor_level = float(np.median(ypre))
        baseline = anchor_level + slope * (distance - anchor_x)
        ...
        if defect_type == 'moisture_local':
            ...
            target_drop = float(np.clip(
                response_scale * (0.82 + 0.76 * severity) * (0.88 + 0.48 * math.sqrt(frac))
        ...
        after = distance > end
        end_level = float(anchor_level + slope * (end - anchor_x) - target_drop)
        target[after] = end_level + post_slope * (distance[after] - end)
...
def suppress_distributed_moisture_micropeaks_s11(
    out: np.ndarray,
    freq_hz: np.ndarray,
    cable: CableSample,
    rng: np.random.RandomState,
    ...
    """Remove numerical taper-boundary needles inside distributed wet regions.

    The distributed RLGC profile is represented by many finite segments.  Even
    when adjacent characteristic impedances are nearly matched, the accumulated
    dispersive boundaries can appear as a train of small negative peaks.  Real
    distributed moisture is a continuous C/G transition, so only the broad,
    low-level component is retained.  The edit is local, smoothly tapered and
    zero-area, preserving both the wet-section step slope and the downstream
    level.
    """
    if params.profile != "field":
        return out
    regions = [
        r for r in _moisture_regions(cable)
        if str(r.get("type", "")) == "moisture_distributed"
    ]
    if not regions:
        return out
    result = np.asarray(out, dtype=np.complex128).copy()
    ...
    for region in regions:
        for iteration in range(3):
            ...
            wet_length = max(end - start, 1.0)
            dx = max(float(np.median(np.diff(distance[: min(len(distance), 4096)]))), 1e-6)
            sigma_m = float(np.clip(0.010 * wet_length, 4.0, 11.0))
            sigma = float(np.clip(sigma_m / dx, 2.0, 180.0))
            radius = max(8, int(math.ceil(4.0 * sigma)))
            xk = np.arange(-radius, radius + 1, dtype=np.float64)
            kernel_g = np.exp(-0.5 * np.square(xk / sigma))
            kernel_g /= max(float(np.sum(kernel_g)), 1e-12)
            smooth = np.convolve(
                np.pad(current, radius, mode="edge"), kernel_g, mode="valid"
            )

            margin = float(np.clip(0.018 * wet_length, 12.0, 28.0))
            active = (distance >= start + 0.25 * margin) & (distance <= end - 0.45 * margin)
            idx = np.flatnonzero(active)
            if len(idx) < 80:
                break
            u = np.linspace(0.0, 1.0, len(idx), dtype=np.float64)
            edge = float(np.clip(margin / max(wet_length, 1.0), 0.025, 0.10))
            taper = _smoothstep01(u / edge) * (1.0 - _smoothstep01((u - (1.0 - edge)) / edge))

            # Retain a small fraction of the measured-like continuous texture,
            # but remove the coherent one-per-segment spike train.
            retain = 0.10 if iteration == 0 else 0.04
            desired = smooth[idx] + retain * (current[idx] - smooth[idx])
            residual = np.zeros_like(current)
            residual[idx] = (desired - current[idx]) * taper

            closure = np.exp(-0.5 * np.square((u - 0.5) / 0.34)) * taper
            area = float(np.sum(residual[idx]))
            residual[idx] -= area * closure / max(float(np.sum(closure)), 1e-12)
            try:
                add = localized_time_kernel_to_s11(
                    freq_hz, residual, window=window
                )
            except Exception:
                break
            add = _scale_additive_kernel_to_budget(
                result, add, percentile_limit=1.10, max_limit=1.22
            )
            result += add
    return result
...
def ensure_terminal_visibility_s11(
    ...
    # desired peak fixed prevents the accidental 1.45^N growth of V2.5.
...
def apply_dirty_model(
    ...
    if params.profile == "field":
        out = apply_field_spectral_family_calibration(
            out, freq_hz, rng, measured_template
        )
    ...
    if params.profile == "field":
        # Reserve broadband headroom for the physical end reflection.  Scaling
        # the already-shaped Field carrier preserves its Re/Im morphology,
        # whereas scaling only the terminal branch made long-cable ends vanish
        # whenever the fixture spectrum happened to approach |S11|=1.
        p995 = max(float(np.nanpercentile(np.abs(out), 99.5)), 1e-9)
        carrier_scale = float(np.clip(0.92 / p995, 0.76, 1.0))
        out *= carrier_scale
        terminal_branch = _scale_additive_kernel_to_budget(
            out, terminal_branch, percentile_limit=1.16, max_limit=1.20
        )
    else:
        terminal_branch = _scale_additive_kernel_to_budget(
    ...
    out += terminal_branch
    if params.profile == "field":
        out = apply_field_nominal_step_slope_s11(
            out, freq_hz, cable, rng, params, window=window
        )
        # The terminal has a non-negligible low-frequency contribution under
        # the Client DC extrapolation.  Apply the macroscopic moisture trend
        # after the terminal so the final exported step retains the wet-section
        # slope.  The moisture correction is held constant before the end and
        # therefore does not create a second terminal peak.
        out = apply_moisture_response_s11(
            out, freq_hz, cable, rng, params, window=window
        )
        out = suppress_distributed_moisture_micropeaks_s11(
            out, freq_hz, cable, rng, params, window=window
        )
        out = finalize_field_short_windows_s11(
            out, freq_hz, cable, rng, params, window=window
        )
...
def generate_band(
    ...
    # V2.7 invariant: these are the only response calculations.  No distance-
