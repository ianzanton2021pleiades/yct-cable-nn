"""External library wrapper for the tuned ShortModel BNC/end detector."""
...
MODEL_INFO = {
    "key": "short_bncnet_v2_ref_ft",
    "name": "ShortBNCNet V2 Ref-FT",
    "description": "5通道短电缆专项模型；输出BNC/疑似阻抗突变与末端置信度。",
    "checkpoint": "ShortModel/experiments/short_bncnet_v2_ref_ft/best.pt",
    "threshold": 0.30,
    "end_threshold": 0.30,
    "prominence": 0.04,
...
def _build_short_features(
    ...
    if min(len(distance), len(impulse), len(step)) < 2:
        raise ValueError("距离域响应点数过少，无法运行 ShortModel")
...
def _load_short_model_class():
    ...
    if spec is None or spec.loader is None:
        raise ImportError(f"无法加载 ShortModel 模型定义: {model_path}")
...
def _run_model(features: np.ndarray, device: Optional[str] = None) -> np.ndarray:
    ...
    if not checkpoint_path.exists():
        raise FileNotFoundError(f"找不到 ShortModel 权重文件: {checkpoint_path}")
    try:
...
def analyze(
    ...
    **_: Any,
    ...
    """Run ShortBNCNet inference and return the GUI's unified result shape."""
    grid, features = _build_short_features(distance, impulse_response, step_response)
    probs = _run_model(features, device=device)
    if probs.shape[0] < 2:
        raise ValueError(f"ShortModel 输出通道数异常: {probs.shape}")
    ...
    result.update(
        {
            "events": defect_events + ([] if end_event is None else [end_event]),
            ...
            "bnc_confidence": plot_bnc,
            "end_confidence_curve": plot_end,
        }
    )
    return result
