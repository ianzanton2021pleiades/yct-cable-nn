"""
s11_generator.py — 增强型 S11 合成生成器

基于参考 ADS 生成器的多段级联传输线模型，加入三大增强：
  (A) 随机化拓扑：随机段数、随机位置/长度/参数
  (B) 校准化真实噪声模型：加性+乘性噪声，参数来自 RG58-74M 实测校准
  (C) 真实接头/缺陷特征：段边界处可注入BNC风格接头反射

频率网格 = Client 标准：9kHz–1GHz，10000点（线性扫频）
"""
...
def _compute_s11_for_cable(
    ...
    # 为每段计算 RLGC → Z0, gamma
    z_load = np.full(n_freq, cable.z_load_open, dtype=complex)

    # 从最后一段向第一段递推
    for seg in reversed(cable.segments):
        epsr = seg.epsr
        log_term = seg.z0_ohm * math.sqrt(epsr) / 60.0
        ratio = math.exp(log_term)

        # 几何参数
        rc_m, rs_m = _target_to_geometry(seg.z0_ohm, epsr, seg.alpha_db_per_m_100mhz)

        # 频域 RLGC
        skin_depth = np.sqrt(2.0 / (omega * MU0 * seg.sigma_cu))
        C = np.full(n_freq, 2.0 * PI * epsr * EPS0 / log_term)
        G = np.full(n_freq, 2.0 * PI * seg.sigma_dielectric / log_term)
        ...
        # 阻抗递推
        exp_term = np.exp(-2.0 * gamma * seg.length_m)
        T = (z_load - z0) / (z_load + z0)
        z_load = z0 * (1.0 + T * exp_term) / (1.0 - T * exp_term)

    s11 = (z_load - cable.z_ref) / (z_load + cable.z_ref)
    return s11
